    ...
    for _, row in df_limpio.iterrows():
        try:
            patient = Patient(
                ...
                imc=float(row['imc']),
                presion_sistolica=int(pd.to_numeric(row['presion_sistolica'], errors='coerce') or 0),
                presion_diastolica=int(pd.to_numeric(row['presion_diastolica'], errors='coerce') or 0),
                frecuencia_cardiaca=int(pd.to_numeric(row['frecuencia_cardiaca'], errors='coerce') or 0),
